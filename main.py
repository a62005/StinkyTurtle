...
# 只在 GCP 環境中自動更新 Dialogflow (使用線程避免阻塞啟動)
if os.environ.get('GAE_ENV', '').startswith('standard'):
    def update_dialogflow_async():
        print("🔄 開始 Dialogflow 更新...")
        time.sleep(5)  # 等待應用程式完全啟動
        try:
            ...
            app_url = f"https://{project_id}.de.r.appspot.com"  # 使用正確的 URL 格式
    ...
    # 在背景線程中執行 Dialogflow 更新
    threading.Thread(target=update_dialogflow_async, daemon=True).start()
    print("🔄 Dialogflow 更新線程已啟動")
else:
    print("ℹ️  非 GCP 環境，跳過 Dialogflow 更新")

# 導出 app 供 GCP 使用
# App Engine 會自動使用這個 app 變數