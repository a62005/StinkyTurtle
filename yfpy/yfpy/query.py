...
class YahooFantasySportsQuery(object):
    ...
    def get_team_week_data(self, team_key, week, is_weekly):
        res = ''
        if is_weekly:
        ...
        name = data["fantasy_content"]["team"][0][2]["name"]
        id_value_dict = {}
        id_value_dict['name'] = name

        id_value_dict['team_id'] = team_key
        
        if is_weekly:
          team_remaining_games = data["fantasy_content"]["team"][1]["team_remaining_games"]
          remaining = team_remaining_games["total"]["remaining_games"]
          completed = team_remaining_games["total"]["completed_games"]
          id_value_dict['Games Played'] = f"{completed}/{remaining + completed}"
        else:
          id_value_dict['Today Played'] = ' '

        for stat in stats:
          id = stat["stat"]["stat_id"]
          # id_name = parse_stat_id(id)
    ...
    def get_team_data_by_stat_id(self, hit, week):
        # res = self.get_response(
        #     f"https://fantasysports.yahooapis.com/fantasy/v2/league/{self.get_league_key()}/teams;"
        # ).text
        # league_info = json.loads(res)
        ...
        for k in range(1, num_teams + 1):
          data = self.get_team_week_data(k, week)
          temp = {}
          name = data['name']
          value = data[hit]
          temp[name] = value
          result['teams'].append(temp)
        return result
    ...
    def get_all_data(self, week, is_weekly):
        ...
        for k in range(1, num_teams + 1):
          ...
          keys = data.keys()
          for key in keys:
            stat = parse_stat_id(str(key))
            if stat is not None:
              if result.get(stat) is None:
                result[stat] = {}

              result[stat][name] = data[key]     
